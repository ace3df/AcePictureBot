...
def is_owner_check(message):
    return message.author.id == '81515803085639680'

def is_owner():
    return commands.check(lambda ctx: is_owner_check(ctx.message))

#async def is_owner_check(bot, message):
#    app_info = await discord_bot.application_info()
#    return ctx.message.author.id == getattr(app_info, 'owner').id

def only_allow_server(allowed_servers):
    return commands.check(lambda ctx: ctx.message.server.id in allowed_servers)
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ('Super Moderator', 'Moderator'), **perms)
